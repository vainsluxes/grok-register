...
class CpaExportSettings:
    ...
    def from_config(cls, config):
        ...
        return cls(
            ...
            # Default: skip local cpa_auths JSON; push tokens to 9Router instead.
            save_auth_file=bool(cfg.get("cpa_save_auth_file", False)),
            push_9router=bool(cfg.get("cpa_push_9router", True)),
...
def export_cpa_xai_for_account(email, password, page=None, cookies=None, sso=None,
    ...
    # Only create auth_dir when we need to write local files (CPA JSON or fail log).
    if settings.save_auth_file or settings.copy_to_hotload:
        settings.auth_dir.mkdir(parents=True, exist_ok=True)

    if settings.save_auth_file:
        log("[cpa] mint OIDC for %s -> %s (+ optional 9router)" % (email, settings.auth_dir))
    else:
        log("[cpa] mint OIDC for %s -> 9router direct (no local CPA file)" % email)
    ...
    result = mint_and_export(
        ...
        save_auth_file=settings.save_auth_file,
    )
    result = _normalize_result(result, email)
    result["save_auth_file"] = settings.save_auth_file
    result["push_9router"] = settings.push_9router
