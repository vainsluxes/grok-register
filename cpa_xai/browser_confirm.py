...
def _cookie_banner_visible(text: str) -> bool:
    ...
    return any(
        ...
        for needle in ("allow all", "privacy preferences", "cookie", "privacy preference")
    )
...
def _wait_turnstile(page: Any, log: LogFn, timeout_sec: float, email: str = "", raise_on_timeout: bool = False) -> bool:
    ...
    while time.time() < deadline:
        ...
        try:
            token_length = page.run_js(
                ...
                try {
                    ...
                    if (val.length >= 80) return val.length;
                    if (window.turnstile && typeof window.turnstile.getResponse === 'function') {
                        return String(window.turnstile.getResponse() || '').trim().length;
                    }
                } catch(e) {}
                return 0;
                """
            ...
            if int(token_length or 0) >= 80:
                log("turnstile passed, token length=%s" % token_length)
                return True
        except Exception:
            pass
        ...
        # 2. Actively click checkbox if inside shadow root or iframe
        try:
            ...
            if challenge_input:
                ...
                if iframe:
                    try:
                        ...
                        btn = body_sr.ele("tag:input") or body_sr.ele("tag:span")
                        if btn:
                            btn.click()
                            log("clicked turnstile checkbox inside shadow root")
                    except Exception:
                        pass
        except Exception:
            pass

        # Standalone turnstile iframe search
        try:
            for ifr in page.eles("css:iframe[src*='challenges.cloudflare.com']"):
                try:
                    body_sr = ifr.ele("tag:body").shadow_root
                    btn = body_sr.ele("tag:input")
                    if btn:
                        btn.click()
                        log("clicked standalone turnstile iframe input")
                except Exception:
                    pass
        except Exception:
            pass
...
def approve_device_code(
    ...
    while time.time() < deadline:
        ...
        if "device/done" in url or "device authorized" in text.lower():
        ...
        if _cookie_banner_visible(text):
            if _dismiss_cookie_banner(page, logger):
                _sleep(0.6)
                continue
        if "/consent" in url or "authorize grok build" in text.lower() or "authorize" in text.lower() or "consent" in url:
            phase = "consent"
            if _cookie_banner_visible(_visible_text(page)):
                if _dismiss_cookie_banner(page, logger):
                    _sleep(0.6)
                    continue
            if _click_exact(page, ["Allow", "Authorize", "Approve", "Authorize Grok Build", "Continue"], logger, real=True):
                _sleep(2.5)
                continue
            try:
                page.run_js(
                    ...
                    const form = forms.find((x) => {
                      ...
                      return text.includes('Grok Build') || text.includes('Allow');
                    }) || document.querySelector('form');
                    if (!form) return;
                    const formText = (form.innerText || '');
                    if (formText.includes('privacy preferences') || formText.includes('allow all') || /cookie/i.test(formText)) return;
                    ...
                    const button = [...form.querySelectorAll('button')].find((b) => {
                      ...
                      return text === 'Allow' || text === 'Authorize' || text === 'Approve';
                    });
        ...
        if page.ele("css:input[name='user_code']", timeout=0.3) and "consent" not in url:
            ...
            if user_code:
                try:
                    ...
                    pass
            if _click_exact(page, ["Continue"], logger, real=False):
                _sleep(2.0)
                continue
            try:
                ...
                pass
        if "redirecting" in text.lower() or ("/account" in url and "sign-in" not in url):
            if _click_exact(page, ["Continue"], logger, real=False):
                _sleep(2.0)
                continue
        if _cookie_banner_visible(text):
            ...
            _sleep(0.4)
        if "login with email" in text.lower() or "continue with email" in text.lower():
            if _click_exact(page, ["Login with email", "Continue with email", "Sign in with email"], logger, real=False):
                _sleep(1.5)
                phase = "email"
                continue
        if page.ele("css:input[type='email']", timeout=0.3) and not page.ele("css:input[type='password']", timeout=0.2):
            ...
            if _click_exact(page, ["Next", "Continue"], logger, real=False):
                _sleep(1.8)
                continue
        if page.ele("css:input[type='password']", timeout=0.3):
            ...
            if not _click_exact(page, ["Sign in", "Log in"], logger, real=True):
                try:
