...
def create_standalone_page(proxy: Optional[str] = None, headless: bool = False, log: Optional[LogFn] = None):
    ...
    try:
        ...
        # Shared options already apply browser_hide_mode (normal/minimize/xvfb)
        # and prepare DISPLAY for xvfb. Pass log so hide mode is visible.
    ...
    if options is None:
        ...
        # Best-effort hide mode even without shared runtime
        try:
            from browser_runtime import apply_browser_hide_options, prepare_browser_display

            prepare_browser_display(log_callback=logger)
            apply_browser_hide_options(options, log_callback=logger)
        except Exception as exc:  # noqa: BLE001
            logger("hide options fallback skipped: %s" % exc)

    if headless:
        # Explicit CPA headless override (separate from browser_hide_mode).
        # Prefer hide_mode=minimize/xvfb over this — CF often breaks headless.
        try:
    ...
    try:
        ...
        try:
            ...
            if (not headless) and get_browser_hide_mode() == "minimize":
                force_minimize_browser(browser, log_callback=logger)
        except Exception as exc:  # noqa: BLE001
            logger("post-launch minimize skipped: %s" % exc)
        _register_mint_browser(browser)
        logger("standalone chromium started DISPLAY=%s" % (os.environ.get("DISPLAY") or "?"))
        return browser, page
    except Exception:
...
def close_standalone(browser: Any) -> None:
    ...
    user_data = getattr(browser, "user_data_path", None)
