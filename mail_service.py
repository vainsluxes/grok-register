...
def cloudflare_get_message_detail(api_base, token, message_id):
    ...
    candidates = [
        f"{api_base}/api/mail/{encoded_id}",
        f"{api_base}{get_cloudflare_path('cloudflare_path_messages', '/messages')}/{encoded_id}",
    ]
    last_err = None
    ...
    while time.time() < deadline:
        ...
        if resend_callback and time.time() >= next_resend_at:
            try:
                resend_callback()
                if log_callback:
                    log_callback("[*] Resend verification code triggered")
            except Exception as exc:
                if log_callback:
                    log_callback(f"[Debug] Failed to trigger resend verification code: {exc}")
            next_resend_at = time.time() + 35
        try:
        ...
        for msg in messages:
            ...
            if not address_matched:
                if log_callback:
                    log_callback(f"[Debug] Skipping non-target message id={msg_id} address={msg_addr} to={recipients}")
                continue
            ...
            try:
                ...
                if log_callback:
                    log_callback(f"[Debug] Cloudflare detail API failed, falling back to list content: {exc}")
            if log_callback:
                log_callback(f"[Debug] Cloudflare received message: {subject}")
            code = extract_verification_code(combined, subject)
            if code:
                if log_callback:
                    log_callback(f"[*] Cloudflare extracted verification code from message: {code}")
                return code
            elif log_callback:
                log_callback(f"[Debug] Message parsed but no verification code found id={msg_id} attempt={seen_attempts[msg_id]}")
        sleep_with_cancel(poll_interval, cancel_callback)
    raise Exception(f"Cloudflare did not receive verification code email within {timeout}s")
...
def cloudmail_get_email_and_token():
    ...
    if not get_cloudmail_api_base():
        raise Exception("Cloud Mail API Base not configured")
    if not get_cloudmail_public_token():
        raise Exception("Cloud Mail Public Token not configured")
    domain = cloudmail_next_domain()
    if not domain:
        raise Exception("Cloud Mail receiving domain not configured")
...
def cloudmail_get_messages(address):
    ...
    try:
        ...
        raise Exception(f"Cloud Mail message API returned non-JSON: {resp.text[:300]}")
    if not isinstance(data, dict):
        raise Exception(f"Cloud Mail message API returned invalid format: {data}")
    result_code = data.get("code")
    if result_code not in (None, 200, "200"):
        raise Exception(
            f"Cloud Mail message API failed: code={result_code}, message={data.get('message', '')}"
        )
...
def cloudmail_get_oai_code(
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if log_callback:
                log_callback(f"[Debug] Cloud Mail failed to fetch message list: {exc}")
            sleep_with_cancel(poll_interval, cancel_callback)
            continue
        if log_callback:
            log_callback(f"[Debug] Cloud Mail message count this round: {len(messages)}")
        for msg in messages:
            ...
            subject = str(msg.get("subject", "") or "")
            if log_callback:
                log_callback(f"[Debug] Cloud Mail received message: {subject}")
            code = extract_verification_code(combined, subject)
            if code:
                if log_callback:
                    log_callback(f"[*] Cloud Mail extracted verification code from message: {code}")
                return code
            if log_callback:
                log_callback(
                    f"[Debug] Cloud Mail message parsed but no verification code found "
                    f"id={msg_id} attempt={seen_attempts[msg_id]}"
                )
        sleep_with_cancel(poll_interval, cancel_callback)
    raise Exception(f"Cloud Mail did not receive verification code email within {timeout}s")
...
def duckmail_get_oai_code(
    ...
    while time.time() < deadline:
        ...
        for msg in messages:
            ...
            if code:
                if log_callback:
                    log_callback(f"[*] Extracted verification code from message: {code}")
                return code
        sleep_with_cancel(poll_interval, cancel_callback)
    raise Exception(f"Did not receive verification code email within {timeout}s")
...
def yyds_get_oai_code(
    ...
    while time.time() < deadline:
        ...
        for msg in messages:
            ...
            if code:
                if log_callback:
                    log_callback(f"[*] YYDS extracted verification code from message: {code}")
                return code
        sleep_with_cancel(poll_interval, cancel_callback)
    raise Exception(f"YYDS did not receive verification code email within {timeout}s")
...
class CloudflareMailClient:
    ...
    def create_address(self, domain="", name=""):
        ...
        if not data:
            raise RuntimeError("%s non-JSON: %s" % (self.create_path, raw))
        address = str(data.get("address", "")).strip()
        jwt = str(data.get("jwt", "")).strip()
        if not address or not jwt:
            raise RuntimeError("%s missing address/jwt: %r" % (self.create_path, data))
        return address, jwt
