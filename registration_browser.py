"""Manage main registration browser lifecycle and implement signup page automation."""
import gc
import random
import re
import secrets
...
def set_birth_date(session, log_callback=None):
    ...
    try:
        ...
        if is_cloudflare_block_response(res):
            return (
                False,
                "set_birth_date blocked by Cloudflare protection on grok.com, HTTP "
                f"{res.status_code}",
            )
...
def set_tos_accepted(session, log_callback=None):
    ...
    try:
        ...
        if is_cloudflare_block_response(res):
            return (
                False,
                "set_tos_accepted blocked by Cloudflare protection on accounts.x.ai, HTTP "
                f"{res.status_code}",
            )
...
def update_nsfw_settings(session, log_callback=None):
    ...
    try:
        ...
        if is_cloudflare_block_response(res):
            return (
                False,
                "update_nsfw_settings blocked by Cloudflare protection on grok.com, HTTP "
                f"{res.status_code}",
            )
...
def start_browser(log_callback=None, use_proxy=True):
    ...
    for attempt in range(1, 5):
        ...
        try:
            ...
            # Post-launch hide: CDP minimize if configured
            try:
                from browser_runtime import force_minimize_browser, get_browser_hide_mode
                if get_browser_hide_mode() == "minimize":
                    force_minimize_browser(browser, log_callback=log_callback)
            except Exception:
                pass
            ...
            if log_callback and attempt > 1:
                log_callback(f"[*] Browser started successfully (attempt {attempt})")
            return browser, page
...
def cleanup_runtime_memory(log_callback=None, reason="periodic cleanup"):
    ...
    try:
        import subprocess
        subprocess.run(["pkill", "-9", "-f", "chrome|chromium|crashpad"], capture_output=True)
    except Exception:
        pass
...
def click_email_signup_button(timeout=10, log_callback=None, cancel_callback=None):
    ...
    while time.time() < deadline:
        ...
        if log_callback:
            log_callback("[Debug] Trying to find 'sign up with email' button...")
...
return candidates[0].text || true;
    ...
    raise Exception("'sign up with email' button not found")
...
def open_signup_page(log_callback=None, cancel_callback=None):
    ...
    def _open_with_current_browser():
        ...
        try:
            ...
            if log_callback:
                log_callback(f"[Debug] URL open error: {e}")
    ...
    try:
        ...
        if browser_started_with_proxy and get_configured_proxy():
            if log_callback:
                log_callback(f"[!] Proxy access to signup page failed, falling back to direct: {e}")
    ...
    if browser_started_with_proxy and page_has_proxy_error(page):
        if log_callback:
            log_callback("[!] Browser page shows proxy error, falling back to direct")
        restart_browser(log_callback=log_callback, use_proxy=False)
        _open_with_current_browser()
...
def fill_email_and_submit(timeout=45, log_callback=None, cancel_callback=None):
    ...
    if log_callback:
        log_callback(f"[*] Email created: {email}")
    deadline = time.time() + timeout
    last_diag_time = 0
    last_reclick_time = 0
    last_snapshot = None
...
return candidates[0].text || true;
        ...
        if state != "filled":
            if log_callback:
                log_callback(f"[Debug] Email input appeared but fill failed: {filled}")
            sleep_with_cancel(0.5, cancel_callback)
            continue
...
return 'enter';
        ...
        if clicked:
            if log_callback:
                detail = f" ({clicked})" if isinstance(clicked, str) else ""
                log_callback(f"[*] Filled email and submitted: {email}{detail}")
...
return 'clicked';
        ...
        if clicked == "clicked" or clicked == "no-button":
            if log_callback:
                log_callback(f"[*] Filled verification code and submitted: {code}")
...
if (nodes.length && typeof nodes[0].click === 'function') nodes[0].click();
    ...
    raise Exception("Turnstile token retrieval failed")
...
return 'filled-no-submit';
            ...
            if isinstance(filled, str) and filled.startswith("wait-cloudflare"):
                ...
                if log_callback:
                    ...
                    log_callback(f"[*] Profile filled, waiting for Cloudflare verification... current token length={token_len}")
                if token_len == "0":
                    pause_seconds = random.uniform(1, 3)
                    if log_callback:
                        log_callback(f"[*] Cloudflare token empty, pausing {pause_seconds:.1f}s before retry")
                    sleep_with_cancel(pause_seconds, cancel_callback)
                now = time.time()
                if wait_cf_since is None:
                    wait_cf_since = now
                # Auto re-use Turnstile component when stuck
                if now - wait_cf_since >= 12 and now - last_cf_retry_at >= 8:
                    if log_callback:
                        log_callback("[*] Cloudflare verification stuck, re-using Turnstile...")
                    try:
...
return String(cfInput.value || '').trim().length;
                            ...
                            if log_callback:
                                log_callback(f"[*] Turnstile re-use complete, backfill length={synced}")
                    except Exception as cf_exc:
                        if log_callback:
                            log_callback(f"[Debug] Turnstile re-use failed: {cf_exc}")
            ...
            if filled in ("ready-to-submit", "filled-no-submit"):
                ...
                log_callback("[Debug] Profile fill failed, retrying...")
                sleep_with_cancel(0.5, cancel_callback)
                continue
...
return 'submitted';
        ...
        if isinstance(submit_state, str) and submit_state.startswith("wait-cloudflare"):
            if log_callback:
                ...
                log_callback(f"[*] Waiting for Cloudflare verification before submit... current token length={token_len}")
            now = time.time()
            if wait_cf_since is None:
                wait_cf_since = now
            if now - wait_cf_since >= 12 and now - last_cf_retry_at >= 8:
                if log_callback:
                    log_callback("[*] Still stuck before submit, re-using Turnstile...")
                try:
                    token = getTurnstileToken(log_callback=log_callback, cancel_callback=cancel_callback)
                    if token:
                        synced = page.run_js(
...
return String(cfInput.value || '').trim().length;
        ...
        if isinstance(submit_state, str) and submit_state.startswith("no-submit-button") and log_callback:
            ...
            log_callback(f"[Debug] Submit button not found, continuing to wait for page stability...{suffix}")
...
return 'final-page-clicked-submit';
                ...
                if log_callback and isinstance(retried, str) and retried.startswith("final-page-wait-cf"):
                    ...
                    if now - last_cf_retry_at >= 10:
                        if log_callback:
                            log_callback("[*] Final page Cloudflare stuck, re-using Turnstile...")
                        try:
                            token = getTurnstileToken(log_callback=log_callback, cancel_callback=cancel_callback)
                            if token:
                                synced = page.run_js(
...
return String(cfInput.value || '').trim().length;
                                ...
                                if log_callback:
                                    log_callback(f"[*] Final page Turnstile re-use complete, backfill length={synced}")
                        except Exception as cf_exc:
                            if log_callback:
                                log_callback(f"[Debug] Final page Turnstile re-use failed: {cf_exc}")
                        last_cf_retry_at = now
            ...
            if log_callback:
                ...
                if message != last_wait_exception_message or now - last_wait_exception_at >= 10:
                    log_callback(f"[Debug] Exception while waiting for sso cookie, continuing: {message}")
    ...
    raise Exception(
        f"Timeout: sso cookie not obtained. Seen cookies: {sorted(last_seen_names)}"
    )
